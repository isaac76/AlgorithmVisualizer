...
void TestSet::testInsert()
{
    ...
    Set<TestData> set(compareTestData);
...
{
    // Create two sets
    Set<TestData> set1(compareTestData);
    Set<TestData> set2(compareTestData);
    ...
    // Create a result set
    Set<TestData> result(compareTestData);
    
    // Perform the union operation
    Set<TestData>::unionSet(&result, &set1, &set2);
    ...
    // Empty set union
    Set<TestData> empty1(compareTestData);
    Set<TestData> empty2(compareTestData);
    Set<TestData> emptyResult(compareTestData);
    
    // Empty U Empty = Empty
    Set<TestData>::unionSet(&emptyResult, &empty1, &empty2);
    QCOMPARE(emptyResult.getSize(), 0);
    
    // Set U Empty = Set
    Set<TestData> nonEmptyResult(compareTestData);
    Set<TestData>::unionSet(&nonEmptyResult, &set1, &empty1);
...
void TestSet::testIntersection()
{
    ...
    Set<TestData>::intersectionSet(&result, &set1, &set2);
    ...
    // Empty ∩ Empty = Empty
    Set<TestData>::intersectionSet(&emptyResult, &empty1, &empty2);
    QCOMPARE(emptyResult.getSize(), 0);
    
    // Set ∩ Empty = Empty
    Set<TestData> nonEmptyResult(compareTestData);
    Set<TestData>::intersectionSet(&nonEmptyResult, &set1, &empty1);
    QCOMPARE(nonEmptyResult.getSize(), 0);
    
    // Disjoint sets
    Set<TestData> disjointSet1(compareTestData);
    Set<TestData> disjointSet2(compareTestData);
    ...
    Set<TestData> disjointResult(compareTestData);
    Set<TestData>::intersectionSet(&disjointResult, &disjointSet1, &disjointSet2);
...
void TestSet::testDifference()
{
    ...
    Set<TestData>::differenceSet(&result, &set1, &set2);
    ...
    Set<TestData> result2(compareTestData);
    Set<TestData>::differenceSet(&result2, &set2, &set1);
    ...
    Set<TestData> empty(compareTestData);
    Set<TestData> emptyResult(compareTestData);
    
    // Empty - Set = Empty
    Set<TestData>::differenceSet(&emptyResult, &empty, &set1);
    QCOMPARE(emptyResult.getSize(), 0);
    
    // Set - Empty = Set
    Set<TestData> fullResult(compareTestData);
    Set<TestData>::differenceSet(&fullResult, &set1, &empty);
    ...
    // Set - Set = Empty
    Set<TestData> sameSetResult(compareTestData);
    Set<TestData>::differenceSet(&sameSetResult, &set1, &set1);
...
void TestSet::testSubset()
{
    ...
    Set<TestData> set1(compareTestData);
    Set<TestData> set2(compareTestData);
    Set<TestData> set3(compareTestData);
    ...
    QVERIFY(Set<TestData>::isSubset(&set1, &set2));  // set1 is a subset of set2
    QVERIFY(!Set<TestData>::isSubset(&set2, &set1)); // set2 is not a subset of set1
    QVERIFY(!Set<TestData>::isSubset(&set1, &set3)); // set1 is not a subset of set3
    QVERIFY(!Set<TestData>::isSubset(&set3, &set1)); // set3 is not a subset of set1
    
    // Test edge cases
    
    // Empty set is a subset of any set
    Set<TestData> emptySet(compareTestData);
    QVERIFY(Set<TestData>::isSubset(&emptySet, &set1));
    QVERIFY(Set<TestData>::isSubset(&emptySet, &set2));
    QVERIFY(Set<TestData>::isSubset(&emptySet, &set3));
    QVERIFY(Set<TestData>::isSubset(&emptySet, &emptySet)); // Empty is a subset of itself
    
    // Any set is a subset of itself
    QVERIFY(Set<TestData>::isSubset(&set1, &set1));
    QVERIFY(Set<TestData>::isSubset(&set2, &set2));
    QVERIFY(Set<TestData>::isSubset(&set3, &set3));
    
    // Additional subset tests
    Set<TestData> set4(compareTestData); // Will be {1, 2} like set1
    TestData* d1_dup3 = new TestData(1);
    TestData* d2_dup3 = new TestData(2);
    set4.insert(d1_dup3, true);
    set4.insert(d2_dup3, true);
    
    QVERIFY(Set<TestData>::isSubset(&set1, &set4)); // Same elements - mutual subsets
    QVERIFY(Set<TestData>::isSubset(&set4, &set1));
}
...
void TestSet::testEqualSet()
{
    ...
    // Test equality
    QVERIFY(Set<TestData>::isEqualSet(&set1, &set2));  // set1 equals set2
    QVERIFY(Set<TestData>::isEqualSet(&set2, &set1));  // set2 equals set1
    QVERIFY(!Set<TestData>::isEqualSet(&set1, &set3)); // set1 does not equal set3
    QVERIFY(!Set<TestData>::isEqualSet(&set3, &set1)); // set3 does not equal set1
    ...
    Set<TestData> empty1(compareTestData);
    Set<TestData> empty2(compareTestData);
    QVERIFY(Set<TestData>::isEqualSet(&empty1, &empty2));
    
    // A set is equal to itself
    QVERIFY(Set<TestData>::isEqualSet(&set1, &set1));
    QVERIFY(Set<TestData>::isEqualSet(&set2, &set2));
    QVERIFY(Set<TestData>::isEqualSet(&set3, &set3));
    
    // Empty set is not equal to non-empty set
    QVERIFY(!Set<TestData>::isEqualSet(&empty1, &set1));
    QVERIFY(!Set<TestData>::isEqualSet(&set1, &empty1));
    
    // Test with different insertion order but same elements
    Set<TestData> set4(compareTestData);
    ...
    QVERIFY(Set<TestData>::isEqualSet(&set1, &set4));
}
